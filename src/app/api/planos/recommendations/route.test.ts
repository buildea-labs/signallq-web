...
const validProfile = { people: 2, devices: 4, streaming4k: true, gaming: false, homeOffice: true, largeUploads: false, alwaysOnDevices: false }
...
describe('POST /api/planos/recommendations', () => {
  ...
  it('repassa o resultado do backend sem recalcular faixa/score/ranking', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn<typeof fetch>().mockResolvedValue(new Response(JSON.stringify(recommendation), { status: 200 })))
    const response = await POST(request({ ibge: '3550308', profile: validProfile }))
