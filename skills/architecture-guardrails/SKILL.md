...
- Stores/engines grandes (ex. `historyStore.ts`, `speedEngine.ts`) são reavaliados a cada US que os expande: se a US adiciona uma responsabilidade nova (nova entidade, novo tipo de dado, novo transporte), extrair em módulo próprio (database, repository, export, selectors, configuração, transporte, cálculo) em vez de acrescentar ao arquivo existente.
